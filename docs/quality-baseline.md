...
- 案件书架收录两套完整五夜内容；每案各有 12 条线索、8 件收藏品、三条联合推理与三个结局。
- `CampaignManifest` 在载入时校验连续章节、全部路线、逐夜配套内容、线索引用、视觉配置和可达结局门槛；通用运行时不按案件 ID 分支。
...
- Zustand 浏览器本地存档与 v14 迁移；旧单案件存档自动归入 `case-001`，`campaignSaves` 隔离两案的章节、证物、推理、结局和案板坐标。`opportunityHistory` 保存白天展示、选择或收起快照，`receivedClueIds` 保存好友送达来源，睡眠会话和 `growthHistory` 保存城市时辰与可选睡隙回声。
...
- 自动测试覆盖两套 manifest 完整性、两案从新档完成五夜、不同结局、切换恢复、存档隔离、旧档迁移、案件感知深链接，以及既有方向／社团／问函／纪念物／人物／植物／时辰／睡隙与结局护栏。
- 服务端可渲染产品标题与社交分享元数据。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成首案和雨中电台的五夜循环，验证案件书架切换、第二案结局、案件感知好友线索与独立存档。390 × 844 路径额外检查案件卡触摸目标、无横向溢出、首夜闭环、长收藏／档案、移动案件板和结案卷宗。`tests/game-store.test.ts` 在内存浏览器存储上覆盖两案主循环、切换隔离、跨案件脏数据过滤、旧结构迁移和三个结局。当前共有 54 个 Vitest 测试与 19 条 Playwright 路径；浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
