...
export function SleepHardwarePanel({ onClose }: { onClose: () => void }) {
  ...
  const [nativeAvailability, setNativeAvailability] = useState<NativeSleepAvailability | null>(null);
  const [nativeAuthorizing, setNativeAuthorizing] = useState(false);
  ...
  const connectedDevice = hardware.mode === "virtual" && hardware.consent?.sourceKind === "virtual"
    ? localize(getVirtualSleepDevice(hardware.consent.sourceId))
    : undefined;
  const connectedBridge = hardware.mode === "bridge" && hardware.consent?.sourceKind === "native"
    ? localize(getSleepBridge(hardware.consent.sourceId))
    : undefined;
  ...
  const authorized = hardware.mode === "virtual"
    && hardware.consent?.sourceKind === "virtual"
    && hardware.consent.sourceId === selectedDevice.id;
  ...
  const connected = Boolean(connectedDevice || connectedBridge);
  const connectedSourceName = connectedDevice?.name ?? connectedBridge?.name;
  const selectedNativeBridgeId = selectedBridge.id === "apple-health" || selectedBridge.id === "health-connect"
    ? selectedBridge.id
    : undefined;
  const nativeSupported = Boolean(
    selectedNativeBridgeId
    && nativeAvailability?.available
    && nativeAvailability.bridgeId === selectedNativeBridgeId,
  );
  const nativeReady = hardware.mode === "bridge"
    && hardware.consent?.sourceKind === "native"
    && hardware.consent.sourceId === selectedNativeBridgeId;
  ...
  useEffect(() => {
    let active = true;
    void getNativeSleepAvailability().then((availability) => {
      if (!active) return;
      setNativeAvailability(availability);
      if (availability.available && availability.bridgeId) setDraftBridgeId(availability.bridgeId);
    });
    return () => {
      active = false;
    };
  ...
  const authorizeSystemSleep = async () => {
    if (!selectedNativeBridgeId || !nativeSupported) return;
    setNativeAuthorizing(true);
    setNativeError("");
    try {
      const granted = await authorizeNativeSleep(selectedNativeBridgeId);
      if (!granted || !hardware.authorizeNativeBridge(selectedNativeBridgeId)) {
        setNativeError(t("没有获得睡眠读取权限。你仍可使用虚拟设备完成全部剧情。"));
        return;
      }
      setJustConnected(true);
    } catch (error) {
      setNativeError(error instanceof Error ? error.message : t("系统睡眠授权暂时不可用。"));
    } finally {
      setNativeAuthorizing(false);
    }
  };
    ...
    <motion.div className="sleep-hardware-layer" data-dialog-layer initial={{ opacity: 1 }} animate={{ opacity: 1 }} exit={{ opacity: 1 }}>
      ...
      <motion.aside ref={panelRef} className="sleep-hardware-panel" role="dialog" aria-modal="true" aria-labelledby="sleep-hardware-title" tabIndex={-1} initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 280 }}>
        ...
        <aside ref={summaryRef} className={`sleep-connection-summary ${connected ? "connected" : ""} ${hardware.activeCapture ? "recording" : ""}`}>
          ...
          <div>
            ...
            <b>{hardware.activeCapture
              ? hardware.activeCapture.sourceKind === "native"
                ? locale === "en" ? `${connectedSourceName ?? "System health"} is waiting for the morning record` : `${connectedSourceName ?? "系统健康仓"}正在等待晨间记录`
                : locale === "en" ? `${connectedSourceName ?? "Virtual device"} is recording` : `${connectedSourceName ?? "虚拟设备"}正在记录`
              : connected
                ? locale === "en" ? `${connectedSourceName} connected` : `${connectedSourceName}已连接`
                : t("尚未连接设备，也能完整游玩")}</b>
        ...
        <div ref={tabsRef} className="sleep-source-tabs" role="tablist" aria-label={t("硬件来源")}>
          ...
          <button type="button" role="tab" aria-selected={tab === "bridge"} onClick={() => setTab("bridge")}><Cloud /> {t("真实桥接")} <small>{nativeAvailability?.available ? t("现在可接") : t("接口预演")}</small></button>
        ...
        {tab === "ambient" ? <AmbientHardwareWorkbench /> : tab === "virtual" ? (
          ...
          <section className="sleep-bridge-workbench">
            <aside className="bridge-prototype-stamp">
              {nativeAvailability?.available ? <Smartphone /> : <Cloud />}
              <div>
                <small>{nativeAvailability?.available ? "SYSTEM SLEEP LINK" : "BRIDGE PROTOTYPE"}</small>
                <b>{nativeAvailability?.available ? t("当前手机可以读取系统睡眠摘要") : t("可以浏览，不会改动当前连接")}</b>
                <p>{nativeAvailability?.available
                  ? t("只请求睡眠读取权限。原始记录留在系统健康仓，Night Shift 只保存本夜汇总。")
                  : t("网页与当前设备不提供系统健康仓；Oura、Fitbit 与厂商 SDK 仍保持预演。")}</p>
              </div>
            </aside>
            ...
            <article className="bridge-permission-map">
              ...
              {nativeSupported ? (
                <>
                  <p><ShieldCheck /> {nativeReady
                    ? t("系统睡眠读取已授权；下一次真实夜班结束时会查询同一时间窗。")
                    : t("授权页由系统显示。拒绝、空数据或延迟同步都不会阻断晨报。")}</p>
                  {nativeError && <p role="alert"><CircleOff /> {nativeError}</p>}
                  {nativeReady
                    ? <button type="button" onClick={hardware.revokeConsent}><Unplug /> {t("撤销 Night Shift 本地连接")}</button>
                    : <button type="button" disabled={nativeAuthorizing || Boolean(hardware.activeCapture)} onClick={() => void authorizeSystemSleep()}>
                      <LockKeyhole /> {nativeAuthorizing ? t("等待系统授权…") : locale === "en" ? `Connect ${selectedBridge.name}` : `连接 ${selectedBridge.name}`} <ChevronRight />
                    </button>}
                </>
              ) : (
                <>
                  <p><CircleOff /> {nativeAvailability?.native
                    ? t("这个健康仓不属于当前平台，或系统服务尚不可用。")
                    : t("真实授权只在安装后的 Android／iOS App 中开放。")}</p>
...
export function SleepHardwareHandoff({ onOpen, dark = false }: { onOpen: () => void; dark?: boolean }) {
  ...
  const virtualReady = mode === "virtual" && consent?.sourceKind === "virtual" && consent.sourceId === selectedDeviceId;
  const nativeReady = mode === "bridge" && consent?.sourceKind === "native" && consent.sourceId === selectedBridgeId;
  const ready = virtualReady || nativeReady;
  const sourceName = virtualReady ? device?.name : nativeReady ? bridge?.name : undefined;
  return (
...
export function SleepHardwareNightTelemetry({ session, progress, onOpen, dark = false }: { session: SleepSession | null | undefined; progress: number; onOpen: () => void; dark?: boolean }) {
  ...
  if (capture.sourceKind === "native") {
    const bridge = localize(getSleepBridge(capture.sourceId));
    return (
      <aside className={`sleep-live-telemetry ${dark ? "dark" : ""}`}>
        <header><span><Activity /><i /></span><div><small>SYSTEM SLEEP WINDOW · {t("系统睡眠时间窗")}</small><b>{locale === "en" ? `${bridge?.name} is ready for the morning sync` : `${bridge?.name}等待晨间同步`}</b></div><button type="button" onClick={onOpen}>{t("查看")}</button></header>
        <p>{t("可以直接锁屏休息。App 不会伪装持续采集；晨间只查询本夜时间窗内已经进入系统健康仓的记录。")}</p>
      </aside>
    );
  }
  const device = localize(getVirtualSleepDevice(capture.sourceId as VirtualDeviceId));
...
export function SleepHardwareMorningReceipt({ sessionId, dark = false }: { sessionId: string | undefined; dark?: boolean }) {
  ...
  const nativeConsent = useSleepHardwareStore((state) => state.consent?.sourceKind === "native");
  const refreshNativeSummary = useSleepHardwareStore((state) => state.refreshNativeSummary);
  const lastSleepSession = useGameStore((state) => state.lastSleepSession);
  const lastRefreshRef = useRef(0);
  const { localize, locale, t } = useI18n();

  useEffect(() => {
    if (summary || !nativeConsent || !sessionId || lastSleepSession?.id !== sessionId) return;
    const refresh = () => {
      const now = Date.now();
      if (now - lastRefreshRef.current < 15_000) return;
      lastRefreshRef.current = now;
      void refreshNativeSummary(lastSleepSession);
    };
    refresh();
    const onVisibility = () => {
  ...
  }, [lastSleepSession, nativeConsent, refreshNativeSummary, sessionId, summary]);

  if (!summary) return null;
  const source = summary.sourceKind === "native"
    ? localize(getSleepBridge(summary.sourceId))
    : localize(getVirtualSleepDevice(summary.sourceId as VirtualDeviceId));
  const metrics = [
  ...
  return (
    <article className={`sleep-morning-receipt ${dark ? "dark" : ""}`}>
      <header><div><small>SOMATIC RECEIPT · {t("感官回执")}</small><h3>{locale === "en" ? `A night left by ${source?.name}` : `${source?.name}留下的一夜`}</h3></div><span><ShieldCheck /> {t("仅本机摘要")}</span></header>
      <div>{metrics.map(([label, value]) => <span key={label}><small>{label}</small><b>{value}</b></span>)}</div>
      <blockquote>“{t(summary.narrative)}”</blockquote>
      <p>{t("置信度")} {Math.round(summary.confidence * 100)}% · {summary.sourceKind === "native" ? `${source?.name}${summary.sourceName ? ` / ${summary.sourceName}` : ""}` : t("虚拟样机")} · {t("非医疗结论")}</p>
    </article>
  );
}
