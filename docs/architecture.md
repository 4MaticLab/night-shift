...
案件板只接受 `src/content/relations.ts` 中定义的无序证物对。玩家点击已解锁证物时同时展开完整详情、城市异议与林渡页边批注，并把它放入联合推理台的 A／B 槽；推理台常驻显示“选第一件—选第二件—核对证词”，选满后点击第三件不会静默替换，必须先明确移除其中一件。画布顶部的线索索引只从“双方均已解锁且尚未确认”的确定性关系派生统计，不写入存档：索引短线和卡片线头以氧化铜绿表示仍有未结关系，选中 A 后当前候选的铜绿变亮，已选 A／B 改用暖金纸框与圆形槽位编号；状态语义同时保留为屏幕阅读器文本，但不在纸卡上张贴说明标签。`connectClues` 只会为合法配对写入 `confirmedRelations`，错误配对不改变存档。已确认关系会生成持久连线、退出待核对索引，并在参与作证的证物侧页回响，继续作为真结局条件的一部分。
