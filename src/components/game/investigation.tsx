      ...
      <div className="board-shell">
        <section className="inference-toolbar" aria-label={t("联合推理操作台")}>
          <div className="inference-toolbar-copy">
            ...
            <b>{inferencePrompt}</b>
            <span><i className="desktop-board-hint">{t("点证物选择；只有图钉可以拖动整理。")}</i><i className="mobile-board-hint">{t("点证物选择；上下滑动可以离开案板。")}</i></span>
          </div>
          ...
          <div className="evidence-slots" aria-live="polite">
            {[0, 1].map((index) => {
              ...
              return clue ? <button type="button" className="evidence-slot filled" aria-label={`${t("移除证物")} ${index === 0 ? "A" : "B"}：${clue.title}`} onClick={() => selectEvidence(clue.id)} key={clue.id}><span>{index === 0 ? "A" : "B"}</span><b>{clue.title}</b><X /></button> : <div className="evidence-slot" key={index}><span>{index === 0 ? "A" : "B"}</span><b>{t("等待选择")}</b></div>;
            })}
          </div>
          <button className="connect-evidence" disabled={selectedClueIds.length !== 2} onClick={submitConnection}><Link2 /> {selectedClueIds.length === 2 ? t("核对这两件证物") : locale === "en" ? `Choose ${2 - selectedClueIds.length} more` : `还需选择 ${2 - selectedClueIds.length} 件`}</button>
