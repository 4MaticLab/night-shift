...
- 案件板通过右侧三步联合推理栏要求玩家选择 A／B 两件证物再核对；桌面端画布和右栏是互不覆盖的网格列，右栏不悬浮且不得产生横向滚动。已选线索与匹配动作在画布右侧纵向完整排列，当前证物档案随后显示，核心论断位于最下方。手机端按画布、推理、档案、核心论断恢复文档流。选满不会静默替换，槽位可单独移除。三条核心论断由确定性证物对校验，错误连接不会写入存档，已确认关系会回响到相关证物侧页。
