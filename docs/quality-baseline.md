...
- Zustand 浏览器本地存档与 v7 迁移；`choiceHistory`、`preparationHistory`、`growthHistory`、`societyHistory` 和 `correspondenceHistory` 分别保留每夜方向、随身物、成长、城市关系与已寄答复快照，Demo 抽屉和 `Shift + D` 快捷键可生成可解释的默认履历。
...
- 自动测试已覆盖十五条方向完整性、社团均匀映射与固定奖励护栏、三层关系累计、九问十八答平衡、同社团最近答复查询、无答复继续、五株植物内容与四阶段边界、安稳睡眠的完整五夜循环、真实夜班持久化重载、旧存档温室与关系历史重建、三个结局与真结局门槛。
- 服务端可渲染产品标题与社交分享元数据。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：分别覆盖 Demo 方向事件与植物成长、首封问函答复、同社团后续余波、归来明信片、城市人情簿与答复履历、雾灯温室、真实交接刷新恢复和两证物核心推论。`tests/game-store.test.ts` 在内存浏览器存储上覆盖五夜方向、随身物、成长、关系与答复历史、无答复继续、迁移、恢复与结局。浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
