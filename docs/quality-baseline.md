...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成首案和雨中电台的五夜循环，验证案件书架切换、第二案结局、案件感知好友线索与独立存档，并覆盖放下纸条的显式 AI 授权、最小请求和晨报回信。首载路径会人为延迟首页主视觉，验证加载幕持续可见、背景不可交互并在资源就绪后放行。英文路径会在桌面端验证语言切换、刷新保留、首夜交接、晨报、案板、收藏与档案无可见汉字，并在 390 × 844 验证交接触摸目标和无横向溢出。其余移动路径额外检查案件卡触摸目标、最大长度无断点纸条、首夜闭环、长收藏／档案、移动案件板、结案卷宗，以及《黑水溪》私酒贩入口的虚拟枕授权、交接、信号、晨报与状态化结局闭环；桌面路径另覆盖虚拟戒指完整回执、桥接预演、真实夜班刷新恢复与晨报幂等。`tests/game-store.test.ts` 在内存浏览器存储上覆盖两案主循环、切换隔离、纸条历史、跨案件脏数据过滤、旧结构迁移和三个结局；`tests/rest-ritual.test.ts` 覆盖确定性回信、访问码 Cookie、无密钥回退、同源／大小护栏、受限枚举输出和 OpenAI-compatible 请求；内容测试覆盖首案英文 ID／规则一致性、沙盒引擎的大学摧毁、私酒交易、农场终局、延迟结算幂等与 v1 存档归一化；`tests/sleep-hardware.test.ts` 覆盖无设备降级、设备能力差异、确定性摘要、桥接预演和撤销。合并最新主线后共有 72 个 Vitest 测试与 29 条 Playwright 路径；浏览器测试使用本机 Chrome channel。
