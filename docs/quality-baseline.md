...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成首案和雨中电台的五夜循环，验证案件书架切换、第二案结局、案件感知好友线索与独立存档。390 × 844 路径额外检查案件卡触摸目标、无横向溢出、首夜闭环、长收藏／档案、移动案件板、结案卷宗，以及《黑水溪》私酒贩入口的交接、延迟探索、晨报与状态化结局闭环；桌面路径另覆盖真实夜班刷新恢复与晨报幂等。`tests/game-store.test.ts` 在内存浏览器存储上覆盖两案主循环、切换隔离、跨案件脏数据过滤、旧结构迁移和三个结局；内容测试覆盖沙盒引擎的大学摧毁、私酒交易、农场终局、延迟结算幂等与 v1 存档归一化。当前共有 57 个 Vitest 测试与 21 条 Playwright 路径；浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
...
- 《黑水溪》专属人物／场景插图和环境音批次尚未制作；当前使用原创 CSS 档案地图与纸本界面。忠实版在额外授权复核前不公开推送或生产部署。
