...
- Zustand 浏览器本地存档与 v11 迁移；除既有夜间历史外，`opportunityHistory` 独立保存白天展示、选择或收起快照，`boardPositions` 保存有限的证物坐标，睡眠会话和 `growthHistory` 保存城市时辰，Demo 抽屉和 `Shift + D` 快捷键可生成可解释的默认履历。
...
- 自动测试已覆盖十五条方向完整性、社团均匀映射与固定奖励护栏、三层关系累计、九问十八答平衡、同社团最近答复查询、九件纪念物分布、稳定选择与五夜无重复、四位人物的章节／资产／证物契约、无答复继续、五株植物内容与四阶段边界、四时辰边界与二十条非评分回声、安稳睡眠的完整五夜循环、真实夜班持久化重载、旧存档温室／关系／纪念物／时辰历史重建、三个结局与真结局门槛。
- 服务端可渲染产品标题与社交分享元数据。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除 Demo 方向事件与植物成长、晨报纪念物揭示、跨夜余波、归档、真实交接刷新恢复和两证物核心推论外，还会从新档无章节跳转地连续完成五夜并选择结局，并验证证物阅档、拖动坐标、刷新恢复与默认布局复位。睡前路径断言林渡主肖像、真实方向／随身物／目的地交接单，并以几何边界证明人物不与标题或交接单重叠；夜班、晨报与收藏路径断言 Demo 夜半时辰、真实时辰刷新保持和城市值更簿；结案路径断言五夜总账、八件实际物证、禁用的后续案件出口，以及档案往返后结局保持。390 × 844 浏览器路径会真实操作首夜闭环、时辰卡、收藏、档案、案件板与完整结案卷宗，并在关键页面断言不存在全局横向溢出。`tests/game-store.test.ts` 在内存浏览器存储上覆盖五夜方向、随身物、成长、关系、答复与纪念物历史、案件板坐标、无答复继续、迁移、恢复与结局。当前共有 44 个 Vitest 测试与 12 条 Playwright 路径；浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
