...
- 案件书架收录三套完整五夜内容；三案各有 12 条线索、8 件收藏品、三条联合推理与三个结局。
...
- Zustand 案件浏览器本地存档与 v16 迁移；旧单案件存档自动归入 `case-001`，`campaignSaves` 隔离三案的章节、证物、推理、结局、案板坐标和放下纸条。第三案显示档案号 003、使用技术 ID `case-004`，已退役 `case-003` 与其他非法活动 ID 仍安全回落到 `case-001`，对应非法 `campaignSaves` 条目会被过滤；旧沙盒 localStorage 不再读取也不主动删除。`opportunityHistory` 保存白天展示、选择或收起快照，`receivedClueIds` 保存好友送达来源，睡眠会话和 `growthHistory` 保存城市时辰与可选睡隙回声。
- 真实夜班以持久化开始时间结算；关闭页面再返回仍能恢复进度，晨报会显示实际时长与派生质量。
- 四类虚拟睡眠硬件拥有独立设备能力、逐项本地授权、夜间信号投影和晨报摘要；三案共用 `SleepSession` 采集边界。设备与桥接浏览采用非破坏性草稿，只有明确授权才原子替换当前来源；采集中禁止换源。未授权、撤销和真实桥接预演均不阻断夜班，存档不保留原始传感器时间序列。
...
- 三案三关完成后各开放一段点击排序的最终接线：首案按 `WHEN → WHO → WHERE`，第二案按 `SENDER → CHANNEL → MESSAGE`，第三案按 `CAUSE → OWNERS → RIGHT`。临时排序不持久化，正确接线 ID 复用 v16 存档；错误、撤回、重置与提示均无惩罚。
...
- 自动测试覆盖三套 manifest 完整性、三案从新档完成五夜、切换恢复、存档隔离、已退役案件回落、旧档迁移、案件感知深链接，以及既有方向／社团／问函／纪念物／人物／植物／时辰／睡隙与结局护栏。
- 服务端可渲染产品标题与社交分享元数据。
