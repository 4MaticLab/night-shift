  ...
  return (
    <div className="sandbox-shell">
      <header className="sandbox-topbar">
        <button type="button" className="sandbox-brand" onClick={onHome}><span aria-hidden="true" /><div><b>夜班侦探</b><small>异地卷宗 · {presentation.caseNumber}</small></div></button>
...
function OriginBriefing({
  ...
  return (
    <main className="sandbox-origin">
      ...
      <header><button type="button" onClick={onHome}><ArrowLeft /> 回到案件书架</button><span>CASE {content.presentation.caseNumber} · {content.presentation.caseTypeLabel}</span></header>
    ...
    <div className="sandbox-map-layout">
      ...
      <aside className="sandbox-location-panel">
        {selected ? <>
          ...
          {latest?.locationId === selected.id && <blockquote className="latest-result"><small>最新归档</small><b>{latest.title}</b><p>{latest.result}</p>{(latest.corruptionDelta !== 0 || latest.threatDelta !== 0) && <span>{latest.corruptionDelta ? `${content.presentation.conditionLabel} +${latest.corruptionDelta}` : ""}{latest.corruptionDelta && latest.threatDelta ? " · " : ""}{latest.threatDelta ? `${content.presentation.threatLabel} +${latest.threatDelta}` : ""}</span>}</blockquote>}
...
function SandboxNight({
  ...
  return (
    <main className="sandbox-night">
      ...
      <header>
        <button type="button" className="sandbox-night-brand" onClick={onHome}><span aria-hidden="true" /><div><b>{content.presentation.nightTitle}</b><small>{session.mode === "demo" ? qualityCopy[session.quality].time : "真实夜班"}</small></div></button>
      ...
      <section className="sandbox-report-grid">
        <article className="sandbox-report-result">
          ...
          <dl>
            ...
            <div><dt>{content.presentation.conditionLabel}变化</dt><dd>{report.corruptionDelta ? `+${report.corruptionDelta}` : "无"}</dd></div>
            <div><dt>{content.presentation.threatLabel}变化</dt><dd>{report.threatDelta ? `+${report.threatDelta}` : "无"}</dd></div>
  ...
  return (
    <div className="sandbox-document-page">
      ...
      <section className="corruption-ledger">
        ...
        <div>{content.corruptionStages.map((stage) => <article className={`${stage.stage <= progress.corruption ? "reached" : ""} ${stage.stage === progress.corruption ? "current" : ""}`} key={stage.stage}><span>{stage.stage}</span><div><b>{stage.name}</b><p>{stage.stage <= progress.corruption ? stage.benefit : "尚未抵达"}</p><small>{stage.stage <= progress.corruption ? stage.cost : content.presentation.conditionAdvanceHint}</small></div></article>)}</div>
      </section>
...
function SandboxEnding({ content, progress, endingId, onHome, onReview, onReset }: { content: SandboxCampaignContent; progress: SandboxProgress; endingId: string; onHome: () => void; onReview: () => void; onReset: () => void }) {
  ...
  return <main className="sandbox-ending-reveal" style={background}><div className="ending-pulse" /><header><span>CASE {content.presentation.caseNumber} · {ending.archiveLabel}</span><button type="button" onClick={onHome}><House /> 案件书架</button></header><section><small>{content.year} · {content.presentation.endingEyebrow}</small><h1>{ending.title}</h1><b>{ending.theme}</b><p>{ending.result}</p><blockquote>{ending.coda}</blockquote><dl><div><dt>调查行动</dt><dd>{progress.completedActionIds.length}</dd></div><div><dt>带回线索</dt><dd>{progress.clueIds.length} / {content.clues.length}</dd></div><div><dt>{content.presentation.conditionLabel}阶段</dt><dd>{progress.corruption} / 7</dd></div><div><dt>救出人物</dt><dd>{rescued}</dd></div></dl><div className="ending-actions"><button type="button" onClick={onReview}><FileText /> 重看证物</button><button type="button" onClick={onReset}><RotateCcw /> 重新调查</button></div></section><footer><span>{content.credits.attribution}</span><span>{content.credits.usage}</span></footer></main>;
}
