...
案件板只接受 `src/content/relations.ts` 中定义的无序证物对。`deriveEvidenceBoardHints` 只从已解锁线索、已确认关系和当前选择派生未结关系：顶部线索索引显示可核对关系数量，细线标出仍参与未结关系的证物，选择 A 后只把与其组成确定性关系的候选提亮；这些提示不写存档，也不会代替玩家确认。玩家点击已解锁证物时同时展开完整详情、城市异议与林渡页边批注，并把它放入联合推理台的 A／B 槽；推理台常驻显示“选第一件—选第二件—核对证词”，选满后点击第三件不会静默替换，必须先明确移除其中一件。`connectClues` 只会在明确点击核对后为合法配对写入 `confirmedRelations`，错误配对不改变存档。成功核对会打开推论信笺，关闭后仍生成持久连线、进入右侧核心论断，并在参与作证的证物侧页回响，继续作为真结局条件的一部分。
...
案件板由顶部线索索引、明确尺寸的证物画布和右侧推理阅档栏组成。桌面端使用真实双栏：左侧索引与画布只负责提示、浏览与整理证物，右栏按“联合推理—当前证物档案—核心论断”排列；A／B 两个已选线索槽与核对动作常驻右栏顶部，核心论断始终是右栏最后一段。证物卡的独立阅档入口和右栏“以信笺阅档”打开同一完整档案；已确认核心论断可重新打开推论信笺。两种信笺都是覆盖层，不替换右栏或增加固定底栏。900 px 以下恢复文档流，390 px 手机端依次显示索引与画布、联合推理、当前档案与核心论断。这样选择状态与提交动作处在同一区域，React Flow 也不会拦截移动页面滚动。
