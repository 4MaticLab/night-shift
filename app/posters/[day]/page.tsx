  ...
  return (
    <main className="poster-site">
      ...
      <div className="poster-print-stack">
        <PosterSheet poster={poster} qrSrc={qrSrc} previewPath={`/posters/${poster.day}`} locale={locale} />
      </div>
      <p className="poster-print-hint">{translateText("这是一张独立 A3 打印页。打印时请选择边距“无”、缩放 100% 并开启背景图形；页面导航不会进入成品。", locale)}</p>
    </main>
  );
}
