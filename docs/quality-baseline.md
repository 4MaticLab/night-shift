...
- 案件书架收录四套完整五夜内容；四案各有 12 条线索、8 件收藏品、三条联合推理与三个结局。
...
- 晨报与结束当日已解耦：当前晨报可直接往返案件板、完成证物配对并保持同一报纸；明确结束当日后，「今晨」仍从历史快照重放最新晨报，刷新后保持。首次无报告仍使用原空态，最终夜仍进入结局。
- Demo 控制台只读打开；所有章节／案件板／结局快照和当前案件重置均逐项确认。Demo、睡眠硬件、好友线索、Injective、证物信笺和推论信笺共享焦点圈、背景 inert、Body 滚动锁定、Escape 关闭与触发器焦点恢复。
- 平板与手机晨报默认收起完整夜班旁支档案，核心日志、路线、新证物、矛盾与下一步保持可达；桌面默认展开。案件板先呈现证物与联合推理，可选密文台位于其后并默认收起，两处都使用原生 `details` 暴露展开状态。
- 冷加载首帧立即显示品牌加载幕，真实等待应用 hydration、字体与当前案件主视觉后才开放交互；主视觉失败或超过 7 秒会安全放行。案件板、夜间循环、结局与硬件中心不再进入首页同步模块。
- Zustand 案件浏览器本地存档与 v16 迁移；旧单案件存档自动归入 `case-001`，`campaignSaves` 隔离四案的章节、证物、推理、结局、案板坐标和放下纸条。第三案显示档案号 003、使用技术 ID `case-004`，第四案显示档案号 004、使用技术 ID `case-005`；已退役 `case-003` 与其他非法活动 ID 仍安全回落到 `case-001`，对应非法 `campaignSaves` 条目会被过滤。`opportunityHistory` 保存白天展示、选择或收起快照，`receivedClueIds` 保存好友送达来源，睡眠会话和 `growthHistory` 保存城市时辰与可选睡隙回声。
...
- 十二条证物均可点击展开完整详情、城市异议与林渡页边批注；桌面案件板允许从图钉拖动整理并在刷新后恢复摆放，也可一键回到默认布局。移动端关闭画布与证物拖动，纵向滑动可以稳定离开画布。
- 案件板通过右侧三步联合推理栏要求玩家选择 A／B 两件证物再核对；顶部索引只读统计双方已解锁且尚未确认的关系，细线标记相关证物，选择 A 后用铜绿高亮确定性兼容候选，但不会自动确认。桌面端已选线索与匹配动作在画布右侧纵向排列，当前证物档案随后显示，核心论断位于最下方。手机端按索引与画布、推理、档案、核心论断恢复文档流，密文台位于整个工作区之后。选满不会静默替换，槽位可单独移除。阅档可展开暖纸信笺，成功核对会揭示推论信笺；两者关闭后仍保留右栏状态。三条核心论断由确定性证物对校验，错误连接不会写入存档，已确认关系会回响到相关证物侧页。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成四案五夜循环，验证案件书架切换、三幕案件序章、未开封案件启动、各案结局、案件感知好友线索与独立存档，并覆盖第三、第四案晨报／结局专属美术。案件序章路径明确核对异象—证物—交接的顺序、返回上一幕、最终接案语义，以及手机／平板无横向溢出和触控尺寸。Injective 路径额外覆盖未配置诚实降级、模拟既有 token 回执、刷新保留，以及 390 × 844、820 × 1180、1440 × 900 无横向溢出与触控尺寸。首载路径会人为延迟首页主视觉，验证加载幕持续可见、背景不可交互并在资源就绪后放行，同时断言首案主卷宗与推荐标识；自动本地化路径验证英文浏览器首帧、三个未翻译案件回退、中文 Cookie 覆盖、刷新保持和旧本地偏好迁移。820 × 1180 路径验证平板竖屏案件板的触控文档流、画布宽度、推理台衔接和无横向溢出；390 × 844 路径验证四张案件入口、触摸目标和无横向溢出。英文完整路径会在桌面端验证案件序章、首夜交接、晨报、案板、收藏与档案无可见汉字，并在 390 × 844 验证交接触摸目标和无横向溢出。其余移动路径额外检查最大长度无断点纸条、首夜闭环、长收藏／档案、移动案件板和结案卷宗；桌面路径另覆盖虚拟戒指完整回执、桥接预演、真实夜班刷新恢复与晨报幂等。`tests/i18n.test.ts` 覆盖 Cookie 优先级、语言地区标签、质量权重、未知语言、序列化和全部准备物英文投影；`tests/game-store.test.ts` 在内存浏览器存储上覆盖默认首案、四案主循环、切换隔离、纸条历史、跨案件脏数据过滤、已退役案件回落、旧结构迁移和结局；内容测试覆盖四案 manifest、三幕序章资产完整性、第三与第四案各 34 张资产消费、首案英文 ID／规则一致性和确定性内容护栏。浏览器测试使用本机 Chrome channel。
