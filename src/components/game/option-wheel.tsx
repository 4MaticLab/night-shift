...
export function OptionWheel({
  ...
  const cfgRef = useRef({} as WheelCfg);
  ...
  const runFrame = useCallback((now: number) => {
    ...
    rafRef.current = settled ? null : requestAnimationFrame(frameFnRef.current);
  }, []);

  useEffect(() => {
    frameFnRef.current = runFrame;
  }, [runFrame]);
  ...
  // Sync props into the config ref after render, then re-run the layout so
  // geometry changes (spacing, tilt, items...) apply without re-mounting.
  useEffect(() => {
  ...
  }, [items, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, loop, smoothing, draggable, soundUrl, soundVolume, applyTarget]);

  useEffect(
    () => () => {
      // Also reset the id so a StrictMode remount can start a fresh loop —
      // startLoop treats a non-null id as "already running".
