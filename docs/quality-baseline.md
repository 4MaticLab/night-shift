...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除 Demo 方向事件与植物成长、晨报纪念物揭示、跨夜余波、归档、真实交接刷新恢复和两证物核心推论外，还会从新档无章节跳转地连续完成五夜并选择结局，并验证证物阅档、拖动坐标、刷新恢复与默认布局复位。390 × 844 浏览器路径会真实操作首夜闭环、收藏、档案、证物侧页和两证物连接，并在关键页面断言不存在全局横向溢出。`tests/game-store.test.ts` 在内存浏览器存储上覆盖五夜方向、随身物、成长、关系、答复与纪念物历史、案件板坐标、无答复继续、迁移、恢复与结局。浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
