...
- 《黑水溪》沙盒包含双入口、九地点、20 条证物、六份手札、12 名 NPC、0–7 污染轨道和五种收场；390 × 844 可从私酒贩入口完成行动、结案、回看证物并返回同一结局。
...
- Zustand 线性案件浏览器本地存档与 v14 迁移；旧单案件存档自动归入 `case-001`，`campaignSaves` 隔离前两案的章节、证物、推理、结局和案板坐标。沙盒另用 `night-shift-sandbox-v1` v1 按案保存地点、行动、人物、污染、威胁和结局。`opportunityHistory` 保存白天展示、选择或收起快照，`receivedClueIds` 保存好友送达来源，睡眠会话和 `growthHistory` 保存城市时辰与可选睡隙回声。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成首案和雨中电台的五夜循环，验证案件书架切换、第二案结局、案件感知好友线索与独立存档。390 × 844 路径额外检查案件卡触摸目标、无横向溢出、首夜闭环、长收藏／档案、移动案件板、结案卷宗，以及《黑水溪》私酒贩入口至状态化结局的闭环。`tests/game-store.test.ts` 在内存浏览器存储上覆盖两案主循环、切换隔离、跨案件脏数据过滤、旧结构迁移和三个结局；内容测试覆盖沙盒引擎的大学摧毁、私酒交易和农场终局分支。当前共有 55 个 Vitest 测试与 20 条 Playwright 路径；浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
