...
- Zustand 浏览器本地存档与 v13 迁移；除既有夜间历史外，`opportunityHistory` 独立保存白天展示、选择或收起快照，`boardPositions` 保存有限的证物坐标，`receivedClueIds` 保存好友送达来源，睡眠会话和 `growthHistory` 保存城市时辰与可选睡隙回声，Demo 抽屉和 `Shift + D` 快捷键可生成可解释的默认履历。
...
- 任一已解锁证物都可生成单条白名单二维码与复制链接；好友深链接只把对应证物写入本地案件板，重复接收幂等、非法 ID 不启动存档，且“好友送达”不会直接满足真结局线索门槛。
- 公开、保护与交还选择权三个结局各有独立的结果、林渡终函和结案语；结案页从存档历史重组五夜路线、随身物、植物、纪念物与实际找到的核心物证，不生成评分或假履历。
- 结案后可进入只读案件档案再返回原结局；“下一宗案件”明确保持禁用，“重新调查”仍是唯一重置入口。
- 自动测试已覆盖十五条方向完整性、社团均匀映射与固定奖励护栏、三层关系累计、九问十八答平衡、同社团最近答复查询、九件纪念物分布、稳定选择与五夜无重复、四位人物的章节／资产／证物契约、无答复继续、五株植物内容与四阶段边界、四时辰边界与二十条非评分回声、五条睡隙回声和同夜不可重复护栏、安稳睡眠的完整五夜循环、真实夜班持久化重载、旧存档温室／关系／纪念物／时辰／睡隙兼容、三个结局与真结局门槛。
- 服务端可渲染产品标题与社交分享元数据。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除 Demo 方向事件与植物成长、晨报纪念物揭示、跨夜余波、归档、真实交接刷新恢复和两证物核心推论外，还会从新档无章节跳转地连续完成五夜并选择结局，并验证证物阅档、拖动坐标、刷新恢复与默认布局复位。睡前路径断言林渡主肖像、真实方向／随身物／目的地交接单，并以几何边界证明人物不与标题或交接单重叠；交互路径额外约束桌面主操作与底部导航不重叠、长页换页／重复点击当前导航回顶、键盘焦点可见，以及移动端关键触摸目标不小于 44 px。夜班、晨报与收藏路径断言 Demo 夜半时辰、真实时辰刷新保持、一次睡隙记录、刷新后回声保留、睡隙回声簿和城市值更簿；好友路径覆盖二维码内容、复制链接、合法接收、重复接收与未知 ID。结案路径断言五夜总账、八件实际物证、禁用的后续案件出口，以及档案往返后结局保持。390 × 844 浏览器路径会以断续 Demo 真实操作首夜闭环与睡隙晨报，再覆盖收藏、档案、案件板与完整结案卷宗，并在关键页面断言不存在全局横向溢出。`tests/game-store.test.ts` 在内存浏览器存储上覆盖五夜方向、随身物、成长、关系、答复、纪念物与睡隙历史、案件板坐标、好友线索、无答复继续、迁移、恢复与结局。当前共有 49 个 Vitest 测试与 16 条 Playwright 路径；浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
