  ...
  const toggleAiRequest = async () => {
    ...
    try {
      ...
      if (access.authorized) {
        ...
        setAiAccessMessage(access.configured ? "输入主办方或部署者提供的访问码。" : "当前部署未配置 AI，仍可使用仅本机回信。");
      }
    } catch {
      setAiAccessOpen(true);
      setAiAccessMessage("暂时无法确认 AI 访问权限，仍可使用仅本机回信。");
    } finally {
  ...
  const unlockAiRequest = async () => {
    ...
    try {
      ...
      if (response.ok && access.authorized) {
        ...
        setAiAccessMessage(access.configured ? "访问码不正确；纸条尚未发送。" : "当前部署未配置 AI，仍可使用仅本机回信。");
      }
    } catch {
      setAiAccessMessage("无法验证访问码；纸条尚未发送。");
    } finally {
      setAiAccessChecking(false);
    }
  };
    ...
    <div className="content-grid tonight-page">
      ...
      <section className="plan-panel">
        ...
        <section className="rest-intention-box" aria-labelledby="rest-intention-title">
          <div className="rest-intention-heading"><PenLine /><div><small>LEAVE IT FOR THE NIGHT · 可选休息仪式</small><b id="rest-intention-title">今晚，有什么可以先不解决？</b></div></div>
          <p>写下一件想暂时放下的事。它不参与睡眠评分、案件结算或奖励，清晨只会换回一张短笺。</p>
          <label htmlFor="rest-intention">放下纸条</label>
          <textarea id="rest-intention" value={restIntention} maxLength={REST_INTENTION_MAX_LENGTH} onChange={(event) => { setRestIntention(event.target.value); if (!event.target.value.trim()) { setAiRequested(false); setAiAccessOpen(false); } }} placeholder="例如：明天的演示还没准备完，但今晚先到这里。" />
          <div className="rest-intention-meta"><span>{restIntention.length}/{REST_INTENTION_MAX_LENGTH}</span><small>默认仅保存在这台设备</small></div>
          <button type="button" className={aiRequested ? "ai-consent selected" : "ai-consent"} aria-pressed={aiRequested} disabled={!restIntention.trim() || aiAccessChecking} onClick={() => void toggleAiRequest()}><span>{aiRequested ? <Check /> : <Sparkles />}</span><div><b>{aiAccessChecking ? "正在确认 AI 访问权限" : "请 AI 替林渡选择晨间短笺风格"}</b><small>仅发送本夜纸条，以及案件／章节标题、地点、方向、随身物和侦探名；AI 只选择受限风格，短笺由安全模板组成。需要部署者访问码。</small></div></button>
          {aiAccessOpen && <div className="ai-access-gate"><label htmlFor="ai-access-code">AI 访问码</label><div><input id="ai-access-code" type="password" autoComplete="off" value={aiAccessCode} maxLength={128} onChange={(event) => setAiAccessCode(event.target.value)} placeholder="由部署者提供" /><button type="button" disabled={!aiAccessCode.trim() || aiAccessChecking} onClick={() => void unlockAiRequest()}>验证并启用</button></div><p role="status">{aiAccessMessage}</p></div>}
        </section>
        <SleepHardwareHandoff onOpen={onHardware} />
        </div>
        <button type="button" disabled={!selectedChoice || phase !== "ready"} className="handoff-button" onClick={() => onLaunch(quality, preparationId, sleepMode, restIntention.trim() ? { intention: restIntention, aiRequested } : undefined)}>{!selectedChoice ? "先选择一个调查方向" : sleepMode === "real" ? "开始今夜的真实交接" : "今晚交给你了"} <Moon size={18} /></button>
      </section>
    </div>
  );
}
...
function WakeEchoSlip({ echo, recordedAt, mode }: { echo: WakeEcho; recordedAt?: string; mode: SleepMode }) {
  const clock = mode === "demo" ? "02:17" : recordedAt ? new Intl.DateTimeFormat("zh-CN", { hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(recordedAt)) : "此刻";
...
export function MorningReport({ onContinue, onHardware }: { onContinue: () => void; onHardware: () => void }) {
  ...
  }, [campaign.case.title, campaign.presentation.detectiveName, campaignId, chapter, completeRestReflection, current.title, preparation?.shortTitle, restRitual, result.direction.destination, result.direction.dispatchTitle]);
  ...
  const restReflectionDisclosure = restRitual?.status === "pending"
    ? "本地回信已先写好；模型完成后会在原位替换。"
    : restRitual?.source === "ai"
      ? "只向模型发送了本夜纸条与已列明的最小叙事上下文。"
      : restRitual?.reason === "local-only"
        ? "内容只在本机生成和保存，没有发起网络请求。"
        : restRitual?.reason === "not-configured"
          ? "纸条到达应用服务端，但服务端未配置模型，因此没有发送给模型。"
          : restRitual?.reason === "access-required"
            ? "纸条到达应用服务端，但访问权限已失效，因此没有发送给模型。"
            : "应用服务端已尝试请求模型，但请求失败或输出未通过校验，因此改用本地回信。";
  return (
    <div className="report-wrap">
      ...
      {wakeEcho && <PaperCard className="wake-echo-report"><div className="wake-report-signal"><Radio /><span>ONE BRIEF WAKING</span></div><div><div className="paper-heading"><small>SLEEP GAP · A NOTE BETWEEN DREAMS</small><b>你短暂醒来时，夜班没有结束</b></div><h3>{wakeEcho.title}</h3><p><b>听见：</b>{wakeEcho.sound}</p><p><b>看见：</b>{wakeEcho.glimpse}</p><blockquote>“{wakeEcho.fieldNote}”</blockquote><footer>这条回声不是奖励，不改变线索、植物、睡眠评价或结局资格。</footer></div></PaperCard>}
