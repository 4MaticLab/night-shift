...
- Zustand 线性案件浏览器本地存档与 v14 迁移；旧单案件存档自动归入 `case-001`，`campaignSaves` 隔离前两案的章节、证物、推理、结局和案板坐标。沙盒另用 `night-shift-sandbox-v1` v2 按案保存世界状态、交接、活动夜班与晨报，并兼容 v1 日间存档。`opportunityHistory` 保存白天展示、选择或收起快照，`receivedClueIds` 保存好友送达来源，睡眠会话和 `growthHistory` 保存城市时辰与可选睡隙回声。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成首案和雨中电台的五夜循环，验证案件书架切换、第二案结局、案件感知好友线索与独立存档。英文路径会在桌面端验证语言切换、刷新保留、首夜交接、晨报、案板、收藏与档案无可见汉字，并在 390 × 844 验证交接触摸目标和无横向溢出。其余移动路径额外检查案件卡触摸目标、首夜闭环、长收藏／档案、移动案件板、结案卷宗，以及《黑水溪》私酒贩入口的虚拟枕授权、交接、信号、晨报与状态化结局闭环；桌面路径另覆盖虚拟戒指完整回执、桥接预演、真实夜班刷新恢复与晨报幂等。`tests/game-store.test.ts` 在内存浏览器存储上覆盖两案主循环、切换隔离、跨案件脏数据过滤、旧结构迁移和三个结局；内容测试覆盖首案英文 ID／规则一致性、沙盒引擎的大学摧毁、私酒交易、农场终局、延迟结算幂等与 v1 存档归一化；`tests/sleep-hardware.test.ts` 覆盖无设备降级、设备能力差异、确定性摘要、桥接预演和撤销。当前共有 63 个 Vitest 测试与 25 条 Playwright 路径；浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
