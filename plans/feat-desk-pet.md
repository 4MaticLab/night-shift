...
- [ ] Electron 子应用骨架：主进程、preload（contextBridge 显式能力面）、双窗口。
- [ ] 纯逻辑库：积分规则、占位传感器快照、虚空摄像头占位分析、睡眠评分。
- [ ] 桌宠前端：待机/睡觉动画、台词气泡、积分 HUD。
- [ ] 面板前端：积分账本、传感器占位卡、录像导入、占位报告。
- [ ] 根仓库集成：npm 脚本、vitest 测试、eslint/tsconfig/.gitignore 隔离。
