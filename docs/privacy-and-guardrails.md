...
真实睡眠设备桥接至少需要：厂商最新文档复核、最小权限评审、令牌不落前端、撤销与删除路径、离线／无数据状态、数据保留周期、平台商店隐私声明，以及真实设备与模拟数据的视觉隔离。Home Assistant 当前只覆盖空间环境设备，不能被宣传为已经接入真实睡眠传感器。任何新增上传、账号或云同步都必须建立新的 `manual` 计划并由用户批准。
