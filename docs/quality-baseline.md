...
- 案件书架收录两套完整五夜内容、一套非商业改编沙盒与一套雾灯城原创沙盒；前两案各有 12 条线索、8 件收藏品、三条联合推理与三个结局。
...
- 《潮汐不肯归档》原创沙盒包含双入口、七地点、22 个行动、16 条证物、五份手札、九件物品、九名 NPC、0–7 回潮与四种收场；首批 1+3+3 插画已进入书架、入口、地点与人物档案，390 × 844 可从家属入口完成七次延迟行动、居民返程、结案与证物回看。
...
- 自动测试覆盖四套 manifest 完整性、前两案从新档完成五夜、两套沙盒的双入口与代表性收场路径、沙盒存档隔离、切换恢复、旧档迁移、案件感知深链接，以及既有方向／社团／问函／纪念物／人物／植物／时辰／睡隙与结局护栏。
- 服务端可渲染产品标题与社交分享元数据。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成首案和雨中电台的五夜循环，验证案件书架切换、第二案结局、案件感知好友线索与独立存档，并覆盖放下纸条的显式 AI 授权、最小请求和晨报回信。英文路径会在桌面端验证语言切换、刷新保留、首夜交接、晨报、案板、收藏与档案无可见汉字，并在 390 × 844 验证交接触摸目标和无横向溢出。其余移动路径额外检查案件卡触摸目标、最大长度无断点纸条、首夜闭环、长收藏／档案、移动案件板、结案卷宗，《黑水溪》私酒贩入口的虚拟枕授权、交接、信号、晨报与状态化结局，以及《潮汐不肯归档》从插画入口、地图、七次交接／晨报、人物档案到居民返程结局的完整路径；桌面路径另覆盖虚拟戒指完整回执、桥接预演、真实夜班刷新恢复、晨报幂等，以及 CASE 004 的主视觉、第七闸地点图、交接、晨报与证物归档。内容测试覆盖首案英文 ID／规则一致性、两套沙盒内容矩阵、代表性结局路径、延迟结算幂等、沙盒隔离与 v1 存档归一化。合并最新主线后共有 74 个 Vitest 测试与 30 条 Playwright 路径；浏览器测试使用本机 Chrome channel。

## 已知边界

- 浏览器端尚未建立像素截图基线或多设备／多浏览器矩阵；当前响应式证据聚焦 390 × 844 Chrome 关键路径。
- 当前本机 Chrome 下，既有桌面图钉拖动回归 `remembers a hand-arranged evidence desk after reload` 没有触发坐标落盘；其余 29 条 Playwright 路径通过，CASE 004 的桌面与移动路径均通过。根因与不降低移动滚动护栏的修复已迁移到 [[plans/0036-evidence-board-drag-e2e-reliability]]。
...
- 《只在雨中播出的电台》、《黑水溪》与《潮汐不肯归档》尚无英文内容；选择英文偏好时书架会明确标记，进入这些案件后安全回退完整中文界面。
