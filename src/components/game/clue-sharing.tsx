  ...
  return <motion.div className="clue-share-scrim" data-dialog-layer role="presentation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <motion.section ref={dialogRef} className="clue-share-dialog" role="dialog" aria-modal="true" aria-labelledby="clue-share-title" tabIndex={-1} initial={{ opacity: 0, y: 18, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: .98 }}>
      ...
      <div className="clue-share-copy">
        ...
        <p>{t("好友扫码或打开链接后，这张证物会进入对方的本地存档。不会附带你的夜班进度、选择或其他线索。")}</p>
      ...
      <div className="clue-share-qr">
        {qrDataUrl ? <Image src={qrDataUrl} alt={locale === "en" ? `QR code sharing “${clue.title}”` : `分享线索「${clue.title}」的二维码`} width={360} height={360} unoptimized /> : <div className="clue-share-qr-loading"><QrCode /><span>{t("正在折叠线索路径…")}</span></div>}
        <small>{t("扫码接收")} · {t("不需要登录")}</small>
      </div>
      <div className="clue-share-link">
        <label htmlFor="clue-share-url">{t("好友线索链接")}</label>
        <input id="clue-share-url" readOnly value={shareUrl} />
        <button type="button" disabled={!shareUrl} onClick={copyLink}>{copyState === "copied" ? <Check /> : <Copy />}{copyState === "copied" ? t("链接已复制") : t("复制链接")}</button>
        {copyState === "error" && <p role="status">{t("浏览器没有允许自动复制，请长按上方链接复制。")}</p>}
      </div>
      <footer><Send /> {t("这是一封只装一张证物的信。好友仍要自己完成联合推理。")}</footer>
    </motion.section>
  </motion.div>;
}
...
export function ClueGiftNotice({ notice, onClose }: { notice: ClueGiftNoticeData; onClose: () => void }) {
  ...
  return <motion.aside className={`clue-gift-notice ${notice.kind}`} role={notice.kind === "error" ? "alert" : "status"} initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
    ...
    <div><small>FRIEND CLUE · {t("好友线索")}</small><b>{notice.title}</b><p>{notice.message}</p></div>
    <button type="button" aria-label={t("关闭好友线索提示")} onClick={onClose}><X /></button>
  </motion.aside>;
}
