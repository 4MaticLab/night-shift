...
export function CaseBoard() {
  const { campaignId, unlockedClueIds, receivedClueIds, confirmedRelations, boardPositions, connectClues, setBoardPosition, resetBoardPositions } = useGameStore();
  ...
  const selectEvidence = useCallback((clueId: string) => {
    ...
    if (selectedClueIds.length === 2) {
      setFeedback({ kind: "error", text: "证物槽已经放满。先移除 A 或 B，再换入另一件证物。" });
      return;
    }
    setFeedback(null);
...
export function Collection() {
  const { campaignId, unlockedCollectibleIds, nightSealIds, chapter, completedReports, preparationHistory, choiceHistory, growthHistory, societyHistory, correspondenceHistory, souvenirHistory, opportunityHistory } = useGameStore();
  const campaign = getCampaign(campaignId);
  ...
  return <div className="collection-page">
    ...
    <section className="city-watch-ledger"><div className="shelf-heading"><small>WHEN THE CITY RECEIVED THE SHIFT · {Object.keys(growthHistory).length}/{nightCount}</small><h3>城市值更簿</h3></div><p>这里保存你在什么时辰把任务交给林渡。它只改变当时仍在街上的人和城市愿意露出的侧面，不改变任何案件成果。</p><div>{campaign.case.chapters.map((entry) => { const record = growthHistory[entry.number]; if (!record) return <article className="watch-ledger-entry locked" key={entry.number}><small>NIGHT 0{entry.number} · UNFILED</small><h4>时辰尚未归档</h4><p>完成这一夜后，交接时刻会留下一段城市侧影。</p></article>; const watch = getCityWatch(record.watchId ?? DEMO_CITY_WATCH_ID); const echo = getCampaignWatchEcho(campaign, entry.number, watch.id); const direction = getCampaignRouteDirection(campaign, entry.number, record.choiceId); return <article className={`watch-ledger-entry watch-${watch.id}`} key={entry.number}><small>NIGHT 0{entry.number} · {watch.archiveLabel}</small><h4>{watch.label}</h4><span>{watch.window} · {direction.destination}</span><blockquote>“{echo.fieldNote}”</blockquote></article>; })}</div></section>
    <section className="sleep-gap-ledger"><div className="shelf-heading"><small>NOTES BETWEEN DREAMS · {wakeEchoCount}/{nightCount}</small><h3>睡隙回声簿</h3></div><p>短暂醒转可以留下声音，但不会让夜班结束。没有醒转的空白同样完整；这里没有需要补齐的收集率。</p><div>{campaign.case.chapters.map((entry) => { const record = growthHistory[entry.number]; if (!record) return <article className="sleep-gap-entry locked" key={entry.number}><small>NIGHT 0{entry.number} · UNRETURNED</small><h4>这一夜尚未归来</h4><p>档案没有替未来的睡意预写内容。</p></article>; if (!record.wakeEchoId) return <article className="sleep-gap-entry quiet" key={entry.number}><small>NIGHT 0{entry.number} · UNDISTURBED</small><h4>夜印里没有裂缝</h4><p>这一夜没有记录醒转。空白不是遗漏，也不比回声少任何成果。</p></article>; const echo = getCampaignWakeEchoById(campaign, record.wakeEchoId); return <article className="sleep-gap-entry returned" key={entry.number}><small>NIGHT 0{entry.number} · ONE BRIEF WAKING</small><h4>{echo.title}</h4><p>{echo.sound}</p><blockquote>“{echo.fieldNote}”</blockquote></article>; })}</div></section>
    <section className="night-greenhouse">
      <div className="shelf-heading"><small>FOGLIGHT GREENHOUSE · {Object.keys(growthHistory).length}/{nightCount}</small><h3>雾灯温室</h3></div>
      <p className="greenhouse-intro">提前醒来不会让任何植物枯死。它只会以更小、更奇异的形态，完整保存那一夜。</p>
      <div className="greenhouse-grid">{campaign.botanicals.map((botanical) => { const record = growthHistory[botanical.chapter]; const direction = record ? getCampaignRouteDirection(campaign, botanical.chapter, record.choiceId) : null; const preparation = record ? getPreparation(record.preparationId) : null; const watch = record ? getCityWatch(record.watchId ?? DEMO_CITY_WATCH_ID) : null; return <article className={record ? `greenhouse-specimen unlocked quality-${record.quality}` : "greenhouse-specimen locked"} key={botanical.id}>{record ? <BotanicalSpecimen chapter={botanical.chapter} compact /> : <div className="greenhouse-locked"><KeyRound /><span>NIGHT 0{botanical.chapter}</span></div>}<div className="greenhouse-copy"><small>{record ? `${watch?.label} · ${qualityCopy[record.quality].label} · ${formatSleepDuration(record.durationMinutes)}` : "SEED DORMANT"}</small><h3>{record ? botanical.name : "种核尚未苏醒"}</h3><p>{record ? botanical.specimenNote : "完成这一夜，无论睡了多久，温室都会保存一株完整植物。"}</p>{record && <span><b>{direction?.dispatchTitle}</b>{preparation?.shortTitle} · {botanical.district}</span>}</div></article>; })}</div>
    </section>
    <section className="journey-album"><div className="shelf-heading"><small>RETURNED POSTCARDS · {completedReports.length}/{nightCount}</small><h3>{campaign.presentation.cityName}寄回的 {nightCount} 个夜晚</h3></div><div className="journey-postcard-grid">{campaign.postcards.map((postcard) => { const unlocked = completedReports.includes(postcard.chapter); const preparationId = preparationHistory[postcard.chapter] ?? "side-lamp"; const preparation = getPreparation(preparationId); const direction = getCampaignRouteDirection(campaign, postcard.chapter, choiceHistory[postcard.chapter] ?? ""); const art = getAsset(postcard.assetId); return <article className={unlocked ? "journey-postcard unlocked" : "journey-postcard locked"} key={postcard.id}><div className="journey-postcard-image">{unlocked ? <Image src={art.src} alt={art.alt} width={768} height={512} /> : <div className="postcard-locked"><KeyRound /><span>NIGHT 0{postcard.chapter}</span></div>}</div><div className="journey-postcard-copy"><small>{unlocked ? postcard.location : "ROUTE NOT RETURNED"}</small><h3>{unlocked ? postcard.title : "尚未寄回"}</h3><p>{unlocked ? postcard.message : `完成这一夜的交接，${campaign.presentation.detectiveName}会从城市里寄回一张明信片。`}</p>{unlocked && <div className="journey-route-history"><small>CHOSEN ROUTE</small><b>{direction.dispatchTitle} · {direction.destination}</b><p>{direction.returnLetter}</p></div>}{unlocked && <span><b>{preparation?.shortTitle ?? "随身物"}</b>{postcard.preparationNotes[preparationId]}</span>}</div></article>; })}</div></section>
    <section className="night-seal-shelf"><div className="shelf-heading"><small>NIGHT SEALS · {nightSealIds.length}/{nightCount}</small><h3>{nightCount} 夜印记</h3></div><div className="night-seal-row">{campaign.case.chapters.map((entry) => { const art = getAsset(getCampaignNightSealAssetId(campaign, entry.number)); const unlocked = nightSealIds.includes(entry.number); return <div className={unlocked ? "night-seal unlocked" : "night-seal locked"} key={entry.number}><Image src={art.src} alt={unlocked ? art.alt : "尚未形成的夜印"} width={160} height={160} /><span>夜 0{entry.number}</span><b>{unlocked ? entry.title : "尚未成形"}</b></div>; })}</div></section>
...
export function ArchivePage() {
  const { campaignId, completedReports, unlockedClueIds } = useGameStore();
  const campaign = getCampaign(campaignId);
  ...
  return <div className="archive-page">
    <div className="page-title"><div><p className="eyebrow">CASE ARCHIVE · {campaign.presentation.archiveNumber}</p><h2>{campaign.case.title}</h2></div><Seal>{progress}% 已查明</Seal></div>
