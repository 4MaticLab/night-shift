...
- 案件书架收录两套完整五夜内容和一套非商业沙盒本地原型；前两案各有 12 条线索、8 件收藏品、三条联合推理与三个结局。
- 《零点四十三分的末班车》提供可选完整英文版，覆盖书架、五夜主循环、共享城市旁支、调查档案与三种结局；语言偏好刷新后保留且不进入游戏存档。
- 《黑水溪》沙盒包含双入口、九地点、20 条证物、六份手札、12 名 NPC、0–7 污染轨道和五种收场；行动已接入交接、12 秒／真实夜班与确定性晨报，390 × 844 可从私酒贩入口完成延迟行动、结案、回看证物并返回同一结局。
- `CampaignManifest` 在载入时校验连续章节、全部路线、逐夜配套内容、线索引用、视觉配置和可达结局门槛；沙盒额外校验地点、行动、条件／效果引用与污染阶段。通用运行时不按案件 ID 写业务分支。
...
- 冷加载首帧立即显示品牌加载幕，真实等待应用 hydration、字体与当前案件主视觉后才开放交互；主视觉失败或超过 7 秒会安全放行。案件板、夜间循环、结局、沙盒与硬件中心不再进入首页同步模块。
- Zustand 线性案件浏览器本地存档与 v15 迁移；旧单案件存档自动归入 `case-001`，`campaignSaves` 隔离前两案的章节、证物、推理、结局、案板坐标和放下纸条。沙盒另用 `night-shift-sandbox-v1` v2 按案保存世界状态、交接、活动夜班与晨报，并兼容 v1 日间存档。`opportunityHistory` 保存白天展示、选择或收起快照，`receivedClueIds` 保存好友送达来源，睡眠会话和 `growthHistory` 保存城市时辰与可选睡隙回声。
- 真实夜班以持久化开始时间结算；关闭页面再返回仍能恢复进度，晨报会显示实际时长与派生质量。
- 四类虚拟睡眠硬件拥有独立设备能力、逐项本地授权、夜间信号投影和晨报摘要；线性案件与《黑水溪》共用 `SleepSession` 采集边界。设备与桥接浏览采用非破坏性草稿，只有明确授权才原子替换当前来源；采集中禁止换源。未授权、撤销和真实桥接预演均不阻断夜班，存档不保留原始传感器时间序列。
...
- 案件板通过右侧三步联合推理栏要求玩家选择 A／B 两件证物再核对；桌面端已选线索与匹配动作在画布右侧纵向排列，当前证物档案随后显示，核心论断位于最下方。手机端按画布、推理、档案、核心论断恢复文档流。选满不会静默替换，槽位可单独移除。三条核心论断由确定性证物对校验，错误连接不会写入存档，已确认关系会回响到相关证物侧页。
...
- 第二案雨中信号台复用同一运行时，加入 `97.3 MHz` 调频锁、`47` 户计费簿运算和 `VOICE` 摩尔斯尾音；三关全解后显示“公共频率已经显影”回执。首案／第二案进度按存档隔离，沙盒案不显示空密文台。
...
- 自动测试覆盖三套 manifest 完整性、前两案从新档完成五夜、沙盒双入口与三条代表性收场路径、切换恢复、存档隔离、旧档迁移、案件感知深链接，以及既有方向／社团／问函／纪念物／人物／植物／时辰／睡隙与结局护栏。
- 服务端可渲染产品标题与社交分享元数据。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除既有 Demo／真实夜班、晨报、联合推理、二维码、拖动持久化和结案往返外，还会从新档分别完成首案和雨中电台的五夜循环，验证案件书架切换、从已有案件返回卷宗库后启动未开封案件、第二案结局、案件感知好友线索与独立存档，并覆盖放下纸条的显式 AI 授权、最小请求和晨报回信。首载路径会人为延迟首页主视觉，验证加载幕持续可见、背景不可交互并在资源就绪后放行，同时断言首案主卷宗与推荐标识；自动本地化路径验证英文浏览器首帧、未翻译案件回退、中文 Cookie 覆盖、刷新保持和旧本地偏好迁移。820 × 1180 路径验证平板竖屏案件板的触控文档流、画布宽度、推理台衔接和无横向溢出；390 × 844 路径验证主卷宗、次级案件触摸目标和无横向溢出。英文完整路径会在桌面端验证首夜交接、晨报、案板、收藏与档案无可见汉字，并在 390 × 844 验证交接触摸目标和无横向溢出。其余移动路径额外检查最大长度无断点纸条、首夜闭环、长收藏／档案、移动案件板、结案卷宗，以及《黑水溪》私酒贩入口的虚拟枕授权、交接、信号、晨报与状态化结局闭环；桌面路径另覆盖虚拟戒指完整回执、桥接预演、真实夜班刷新恢复与晨报幂等。`tests/i18n.test.ts` 覆盖 Cookie 优先级、语言地区标签、质量权重、未知语言、序列化和全部准备物英文投影；`tests/game-store.test.ts` 在内存浏览器存储上覆盖默认首案、两案主循环、切换隔离、纸条历史、跨案件脏数据过滤、旧结构迁移和三个结局；`tests/rest-ritual.test.ts` 覆盖确定性回信、访问码 Cookie、无密钥回退、同源／大小护栏、受限枚举输出和 OpenAI-compatible 请求；内容测试覆盖首案英文 ID／规则一致性、沙盒引擎的大学摧毁、私酒交易、农场终局、延迟结算幂等与 v1 存档归一化；`tests/sleep-hardware.test.ts` 覆盖无设备降级、设备能力差异、确定性摘要、桥接预演和撤销。同步后的主线共有 79 个 Vitest 测试与 33 条 Playwright 路径；浏览器测试使用本机 Chrome channel。
...
- 《只在雨中播出的电台》与《黑水溪》尚无英文内容；选择英文偏好时书架会明确标记，进入这些案件后安全回退完整中文界面。
