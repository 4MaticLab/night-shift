...
- Zustand 浏览器本地存档与 v8 迁移；`choiceHistory`、`preparationHistory`、`growthHistory`、`societyHistory`、`correspondenceHistory` 和 `souvenirHistory` 分别保留每夜方向、随身物、成长、城市关系、已寄答复与纪念物快照，Demo 抽屉和 `Shift + D` 快捷键可生成可解释的默认履历。
...
- 自动测试已覆盖十五条方向完整性、社团均匀映射与固定奖励护栏、三层关系累计、九问十八答平衡、同社团最近答复查询、九件纪念物分布、稳定选择与五夜无重复、无答复继续、五株植物内容与四阶段边界、安稳睡眠的完整五夜循环、真实夜班持久化重载、旧存档温室／关系／纪念物历史重建、三个结局与真结局门槛。
- 服务端可渲染产品标题与社交分享元数据。
