...
《潮汐不肯归档》河下区支线用一张 2:1 低潮主视觉、三张 3:2 地点版画与三张 4:5 人物肖像验证混合语法：主视觉保留左侧标题区，地点图只增强机械、档案与空间感，人物用职业道具区分立场；地图、行动条件、回潮与人物状态仍由 HTML/CSS 承担。红色屋顶粉线和盐晶海绿是该支线的局部识别色，不改变全局深蓝、暖纸和旧金体系。提示词与文件映射见 [[docs/art-prompts/tide-refused-assets]]。

后续版本新增 story thread 时必须先继承全局媒介锚点：纸张颗粒、编辑蚀刻、深墨蓝、旧黄铜、克制琥珀光、统一字体和响应式 HTML 信息层。每条线最多增加一组有叙事原因的局部识别色与一种重复构图母题；不得另起一套边框、角色比例、写实度或 UI 语言。新资产批次需在 `docs/art-prompts/` 记录共用锚点、局部差异、文件映射和输出处理，以便版本更新保持角色、城市与媒介连续性。
