...
- Zustand 浏览器本地存档与 v10 迁移；除既有夜间历史外，`opportunityHistory` 独立保存白天展示、选择或收起快照，`boardPositions` 保存有限的证物坐标，Demo 抽屉和 `Shift + D` 快捷键可生成可解释的默认履历。
...
Playwright Happy Path 位于 `tests/e2e/night-shift.spec.ts`：除 Demo 方向事件与植物成长、晨报纪念物揭示、跨夜余波、归档、真实交接刷新恢复和两证物核心推论外，还会从新档无章节跳转地连续完成五夜并选择结局，并验证证物阅档、拖动坐标、刷新恢复与默认布局复位。睡前路径断言林渡主肖像、真实方向／随身物／目的地交接单，并以几何边界证明人物不与标题或交接单重叠；结案路径断言五夜总账、八件实际物证、禁用的后续案件出口，以及档案往返后结局保持。390 × 844 浏览器路径会真实操作首夜闭环、收藏、档案、案件板与完整结案卷宗，并在关键页面断言不存在全局横向溢出。`tests/game-store.test.ts` 在内存浏览器存储上覆盖五夜方向、随身物、成长、关系、答复与纪念物历史、案件板坐标、无答复继续、迁移、恢复与结局。当前共有 41 个 Vitest 测试与 12 条 Playwright 路径；浏览器测试使用本机 Chrome channel，避免依赖额外下载的 Playwright 浏览器。
