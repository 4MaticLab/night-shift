  ...
  return (
    <div className="content-grid tonight-page">
      <section className={selectedDirection ? "desk-scene handoff-ready" : "desk-scene"}>
        <div className="scene-copy"><p className="eyebrow"><Clock3 size={14} /> {t("今晚的任务 · 23:40 前交接")}</p><h2>{locale === "en" ? <>Lin Du is arranging<br />tonight&apos;s equipment.</> : <>林渡正在整理<br />今晚的装备。</>}</h2><p>{t("选择一个调查方向。无论你今晚睡得如何，故事都会继续。")}</p></div>
...
export function NightRun({ onFinish, onHardware }: { onFinish: () => void; onHardware: () => void }) {
  ...
  return (
    <main className="night-run">
      ...
      <div className="night-seal-growth" aria-label={locale === "en" ? `Night ${chapter} seal forming` : `第${chapter}夜的夜印正在形成`}><Image className="seal-ghost" src={nightSeal.src} alt="" width={118} height={118} /><span style={{ height: `${progress}%` }}><Image src={nightSeal.src} alt={nightSeal.alt} width={118} height={118} /></span></div>
