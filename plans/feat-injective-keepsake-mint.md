...
- 采用 Injective EVM Testnet（chain ID `1439`）和标准 ERC-721 合约，提供可编译、可部署的 `NightShiftKeepsake` 合约与 ABI。
- 新增服务端 mint API：校验案件／收藏白名单、钱包地址和幂等键，用服务端测试网私钥签名并广播合约 mint，返回交易哈希与浏览器链接。
...
- [x] 创建计划并作为分支第一笔提交。
- [ ] 固化 Injective EVM 网络、合约 ABI、元数据与请求／响应契约。
- [ ] 实现服务端签名广播、白名单校验、幂等和测试。
- [ ] 实现钱包连接、本地回执与收藏柜 mint UI。
- [ ] 完成 Solidity 合约、部署脚本和配置说明。
- [ ] 补齐桌面／移动响应式、美术细节与可访问状态。
- [ ] 更新稳定文档并完成全量自动化／浏览器回归。
- [ ] 记录最终证据并在 PR 前退役本计划。

## 验收标准

- 只有当前案件 manifest 中的收藏品 ID 能进入 mint 请求，非法案件／收藏／地址被拒绝。
- 私钥只从服务端 `INJECTIVE_MINTER_PRIVATE_KEY` 读取，客户端 bundle、响应和错误信息均不包含密钥。
- 配置齐全时服务端在 Injective EVM Testnet 广播 ERC-721 mint 并返回可打开的 Blockscout 交易链接；相同钱包／收藏／幂等键不会重复广播。
...
- 2026-07-24：服务端直接签名并广播 owner-only mint，前端只提供接收地址并获得 tx hash；这满足低摩擦演示，同时避免把密钥或可复用签名材料发到浏览器。
